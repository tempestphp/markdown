...
final class FrontMatterRuleTest extends TestCase
{
    ...
    {
        $result = new Parser([new FrontMatterRule(), new NewLineRule(), new ParagraphRule()])->parse(<<<'MD'
        ---
        title: Hello
        foo: bar
        ---
        ...
        $this->assertSame(['title' => 'Hello', 'foo' => 'bar'], $result->frontMatter);
        $this->assertSame('<p>Bar</p>', $result->html);
    }
    ...
    {
        try {
            $tokens = new Parser([new FrontMatterRule(), new NewLineRule(), new ParagraphRule()])->parse(<<<'MD'
            ---
            just text
            ---
    ...
    {
        try {
            new Parser([new FrontMatterRule()])->parse(<<<'MD'
            ---
            title: "Introduction
            ---
            MD);
